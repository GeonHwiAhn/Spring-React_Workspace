...
function App() {
  ...
  useEffect(() => {
    모든유저보기(); // users = 유저목록에 유저가 추가되거나 삭제될 경우 모든 유저 다시보기가 됨(새로고침) 
...
/*
 ...
 /*
  ...
  /********** 유저 추가 버튼 ***********/
  //async await 사용해서 유저 추가히기 addUser에서 가져온 user 한 명을 넣어주기
...
/********** 유저 삭제 버튼 ***********/
   ...
   */
    ...
    /**
     * 자바 컨트롤러에서 @DeleteMapping("/{id}") 매개변수이자 파라미터에 (@PathVariable int id) 작성
    ...
    user.id != id = user.id 유저 아이디와 id(삭제하고자하는 유저아이디)가 일치하지 않으면
    setUsers(새로운유저목록)에 포함시키고
    id(삭제하고자하는유저아이디)와 user.id가 일치하는 아이디는 삭제한다음

    setUsers(새로운유저목록)을 완성시킨다
...
/********** 유저 수정을 완료하면 유저 목록에 수정된 유저를 전달 ***********/
  ...
  const cancelEdit = () => {
    setUserToEdit(null); //유저정보 수정 취소 할 때 null 빈값으로 변경하는 트릭
  }
