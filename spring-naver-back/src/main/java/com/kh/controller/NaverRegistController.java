...
public class NaverRegistController {
	...
	@PostMapping("/NaverAPI/register") //프론트와 데이터를 주고받기 위한 만남의 장소 url api
	public String insertNaverUser(@RequestBody NaverUser naverUser) {
		// DB에 React로 가져온 naverUser정보를 큰 수정 없이 전체 다 넣겠다.
		naverUserService.insertNaverUser(naverUser);
		//naverUserService.insertNaverUser(null);
		// null이 들어갈 자리에는 React에서 받아온 값을 넣어주는 공간
		// 처음에는 Java에서 어떤 값을 넣어줘야할지 모르기 때문에 null로 설정이 되어있는 것일 뿐
		// null 자리에는 @RequestBody 나 @RequestParam으로 가져온 값을 작성
		// @RequsetBody = 전체 (=전체를 한번에 집어넣는다는 것은 부분적으로 수정하거나 커스텀이 필요하지 않을 경우)
		// @RequsetParam = 부분수정 추가, 부분적으로 추가할 때 사용
