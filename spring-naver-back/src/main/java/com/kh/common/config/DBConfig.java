...
public class DBConfig {
	...
	@Autowired
	//import org.apache.catalina.core.ApplicationContext; XXXXXXXXXXXX
	private ApplicationContext applicationContext; //연결되는 주소 관리자 나중에 xml과 같은 경로로 보유하과 관리
	...
	public SqlSessionFactory sessionFactory(DataSource dataSource) throws Exception {
		...
		sfb.setTypeAliasesPackage("com.kh.dto"); //나중에 본인의 dto 패키지명으로 변경, dataBase에 작성한 컬럼값과 dto에 작성한 변수명 대조
	...
	@Bean // commit rollback과 같은 수정하거나 삭제하거나 추가했을 때 완전하게 DB에 저장하거나 되돌릴 수 있도록 도와줌
	public DataSourceTransactionManager dataSourceTransactionManager(DataSource ds) {
		return new DataSourceTransactionManager(ds);
		//insert delete update 후 commit을 하지 않으면 완벽하게 저장이 안된 상태에서 select를 진행하기 때문에
		//저장을 안해서 안보이는 상황이라 여기지 않고, 코드 상이나 흐름에 문제가 있다고 생각할 수 있기 때문에 commit 진행매니저 생성
	}
